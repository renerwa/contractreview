...
export function ContractResultPage({ documentId }: { documentId: string }) {
  ...
  }, [loadResult, result?.analysisResult?.status, result?.reviewTask?.status, result?.reviewTask?.taskId]);
  ...
  const riskDetails = report?.review_details || parsedRiskItems?.review_details || [];
  ...
  const totalRiskScore = Number(reportOverview?.total_risk_score ?? result?.analysisResult?.riskScore ?? 0);
  const keyConclusion =
  ...
  const documentName = String(result?.document?.fileName || 'Contract document');
  const reportNumber = String(result?.analysisResult?.id || documentId).slice(0, 12).toUpperCase();
  ...
  return (
    <div className="contract-report-page mx-auto max-w-6xl px-4 py-10">
      <section className="contract-report-print-header hidden print:block">
        <div className="space-y-2">
          <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
            AI Contract Review Report
          </div>
          <h1 className="text-3xl font-semibold text-slate-950">Contract review report</h1>
          <div className="grid gap-2 text-sm text-slate-600 sm:grid-cols-2">
            <div>Report number: {reportNumber}</div>
            <div>Document: {documentName}</div>
            <div>Generated at: {generatedAt || 'Generated on demand'}</div>
            <div>Overall risk: {String(result?.analysisResult?.riskLevel || 'Processing')}</div>
            <div>Total risk score: {Number.isFinite(totalRiskScore) ? totalRiskScore : '-'}</div>
            <div>Output language: {String(result?.reviewSetup?.outputLanguage || '-')}</div>
          </div>
        </div>
      </section>
        ...
        <Card className="overflow-hidden border-border/70 bg-gradient-to-br from-background via-background to-muted/30 shadow-sm">
          <CardHeader className="gap-6">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-3">
                  <div className="bg-primary/10 text-primary flex size-11 items-center justify-center rounded-2xl border border-primary/15">
              ...
              <div className="grid min-w-[260px] gap-3 sm:grid-cols-2 lg:grid-cols-1 lg:justify-items-end">
                ...
                <div className="rounded-2xl border border-border/70 bg-background/85 p-4 text-sm backdrop-blur-sm lg:max-w-[280px]">
                  <div className="flex items-center gap-2 font-medium">
                    <Sparkles className="text-primary size-4" />
                    Review snapshot
                  </div>
                  <p className="text-muted-foreground mt-2 leading-6">
                    Structured for legal teams: overall risk first, then clause-level findings and revision suggestions.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <SummaryCard title="Risk score" value={Number.isFinite(totalRiskScore) ? String(totalRiskScore) : '-'} accent="score" />
              <SummaryCard title="Overall risk" value={riskLevel} accent={riskLevel} />
              <SummaryCard title="High risk" value={String(highCount)} accent="high" />
              <SummaryCard title="Medium risk" value={String(mediumCount)} accent="medium" />
              <SummaryCard title="Low risk" value={String(lowCount)} accent="low" />
            </div>
      ...
      <section className="hidden print:block">
        <div className="grid gap-4 md:grid-cols-5">
          <PrintSummaryCard title="Risk score" value={Number.isFinite(totalRiskScore) ? String(totalRiskScore) : '-'} />
          <PrintSummaryCard title="Overall risk" value={String(result?.analysisResult?.riskLevel || 'Processing')} />
        ...
        <div className="mt-6 rounded-xl border border-slate-200 bg-white p-5">
          <div className="text-sm font-semibold text-slate-900">Key conclusion</div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px] print:grid-cols-1">
          ...
          <CardContent>
            {reviewStatus !== 'completed' ? (
              <div className="text-muted-foreground flex min-h-[320px] flex-col items-center justify-center gap-3 text-sm">
                <Loader2 className="size-5 animate-spin" />
                <p>Review is still running. This page will refresh automatically.</p>
              </div>
              ...
              <Accordion type="single" collapsible className="w-full print:hidden">
                {combinedRiskItems.map((item: any, index: number) => {
                  ...
                  const level = String(
                    item?.status || item?.risk_level || item?.severity || item?.level || 'unknown'
                  );
                  ...
                  return (
                    <AccordionItem key={`${titleText}-${index}`} value={`risk-${index}`}>
                      <AccordionTrigger className="hover:no-underline">
                        <div className="flex w-full flex-col items-start gap-3 pr-4 text-left">
                          <div className="flex items-center gap-3">
                            ...
                            <Badge variant="secondary" className="rounded-full px-2.5 py-0.5 uppercase tracking-wide">
                              {level}
                            </Badge>
                          </div>
                          <p className="text-muted-foreground text-sm leading-6">{conclusion || 'No summary available.'}</p>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent className="space-y-4 pt-1">
                        <RiskSection title="Original text" content={originalText} />
                        <RiskSection title="Risk explanation" content={description || conclusion} />
                        <RiskSection title="Suggested revision" content={suggestion} />
                      </AccordionContent>
                    </AccordionItem>
                  );
                })}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        <h3 className="text-base font-semibold text-slate-950">{titleText}</h3>
                      ...
                      <div className="mt-4 grid gap-4">
                        <PrintSection title="Original text" content={originalText} />
                        <PrintSection title="Risk explanation" content={description || conclusion} />
                        <PrintSection title="Suggested revision" content={suggestion} />
                      </div>
                    </div>
                  );
                })}
        ...
        <Card className="border-border/70 bg-background/95 shadow-sm print:border-slate-200 print:shadow-none">
          <CardHeader>
            ...
            <CardDescription>
              The report includes the summary and review preferences used for this run.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-2xl border border-border/70 bg-muted/20 p-4">
              ...
              <div className="mt-2 text-sm leading-6 text-muted-foreground">
                The exported report includes the contract summary and the review settings used for this review run.
              </div>
            </div>
            <ContextItem label="Contract summary" value={String(result?.analysisResult?.summary || '')} />
            <Separator />
            <ContextItem label="Perspective" value={String(result?.reviewSetup?.perspective || result?.document?.userParty || 'neutral')} />
            <ContextItem label="Signing country" value={String(result?.reviewSetup?.signingPlace || result?.document?.signingPlace || '')} />
            <ContextItem label="Output language" value={String(result?.reviewSetup?.outputLanguage || '')} />
            <ContextItem label="Focus areas" value={String(result?.reviewSetup?.focusPoints || result?.document?.focusPoints || '')} />
          </CardContent>
        </Card>
      </div>
    </div>
...
function SummaryCard({
  ...
  return (
    <div className={`rounded-2xl border bg-gradient-to-br p-5 shadow-sm ${accentClass}`}>
...
function RiskSection({ title, content }: { title: string; content: string }) {
  return (
    <div className="space-y-2">
      ...
      <div className="text-muted-foreground rounded-lg border bg-muted/20 p-3 text-sm leading-6">
...
function RiskLevelDot({ level }: { level: string }) {
  ...
  return <span className={`inline-flex size-2.5 rounded-full shadow-[0_0_0_4px] ${className}`} />;
}
...
function PrintSection({ title, content }: { title: string; content: string }) {
  return (
    <div>
      ...
      <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm leading-7 text-slate-700 whitespace-pre-wrap">
        {content || 'Not available.'}
      </div>
    </div>
  );

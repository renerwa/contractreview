...
export const envConfigs: ConfigMap = {
  ...
  app_name: process.env.NEXT_PUBLIC_APP_NAME ?? 'ContractReview AI',
  ...
  // 保留独立迁移 schema，避免和业务表 schema 混在一起。
  ...
  // 避免把 package.json 的开发元数据打进客户端包，只暴露产品需要展示的版本号。
  version: process.env.NEXT_PUBLIC_APP_VERSION ?? '1.8.2',
